...
function PdfViewer({ url, name }: { url: string; name: string }) {
  ...
  return <div className="flex h-full min-h-0 flex-col">
    ...
    <div ref={viewportRef} className="min-h-0 flex-1 overflow-auto bg-slate-200 p-1.5 sm:p-4">
      {error ? <PreviewError onRetry={() => { setError(false); setPage(1) }} /> : <PdfDocument file={url} onLoadSuccess={({ numPages }) => setPages(numPages)} onLoadError={() => setError(true)} loading={<LoadingState label="Loading PDF…" />}><Page pageNumber={page} width={pageWidth} scale={scale} rotate={rotation} renderTextLayer renderAnnotationLayer className="pdf-page mx-auto bg-white shadow-xl" /></PdfDocument>}
    </div>
  </div>
}
...
function DocxViewer({ url }: { url: string }) {
  ...
  useEffect(() => { let active = true; const controller = new AbortController(); fetch(url, { signal: controller.signal }).then((response) => { if (!response.ok) throw new Error("Document unavailable"); return response.arrayBuffer() }).then(async (buffer) => { const { renderAsync } = await import("docx-preview"); if (active && ref.current) { ref.current.innerHTML = ""; await renderAsync(buffer, ref.current, undefined, { className: "docx-preview", breakPages: true, ignoreWidth: true, ignoreHeight: true }) } }).catch(() => active && !controller.signal.aborted && setError(true)); return () => { active = false; controller.abort() } }, [url])
  return error ? <PreviewError onRetry={() => setError(false)} /> : <div ref={ref} className="docx-viewer h-full w-full overflow-auto bg-slate-200 p-1.5 sm:p-4" />
}

function SheetViewer({ url, type }: { url: string; type: DocumentType }) { const [html, setHtml] = useState(""); const [error, setError] = useState(false); useEffect(() => { const controller = new AbortController(); fetch(url, { signal: controller.signal }).then((response) => { if (!response.ok) throw new Error("Spreadsheet unavailable"); return response.arrayBuffer() }).then(async (buffer) => { const XLSX = await import("xlsx"); const book = XLSX.read(buffer, { type: "array" }); const firstSheet = book.SheetNames[0]; if (!firstSheet) throw new Error("Spreadsheet is empty"); setHtml(XLSX.utils.sheet_to_html(book.Sheets[firstSheet])) }).catch(() => !controller.signal.aborted && setError(true)); return () => controller.abort() }, [url]); return error ? <PreviewError onRetry={() => { setError(false); setHtml("") }} /> : <div className="h-full overflow-auto bg-white p-4"><div className="spreadsheet-preview" dangerouslySetInnerHTML={{ __html: html }} /><p className="mt-3 text-xs text-slate-500">{type.toUpperCase()} preview · first sheet</p></div> }
function TextViewer({ url }: { url: string }) { const [text, setText] = useState(""); const [error, setError] = useState(false); useEffect(() => { const controller = new AbortController(); fetch(url, { signal: controller.signal }).then((response) => { if (!response.ok) throw new Error("Text unavailable"); return response.text() }).then(setText).catch(() => !controller.signal.aborted && setError(true)); return () => controller.abort() }, [url]); return error ? <PreviewError onRetry={() => { setError(false); setText("") }} /> : <pre className="h-full overflow-auto whitespace-pre-wrap break-words bg-slate-950 p-5 font-mono text-sm leading-6 text-slate-100">{text || "Loading text…"}</pre> }
...
function DocumentViewer({ document, onClose }: { document: DocumentRecord; onClose: () => void }) {
  ...
  return <div className={`fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-1 sm:p-5 ${fullscreen ? "p-0" : ""}`} role="dialog" aria-modal="true" aria-labelledby="document-viewer-title"><div className={`flex h-full max-h-[calc(100dvh-0.5rem)] w-full max-w-6xl flex-col overflow-hidden bg-white shadow-2xl sm:max-h-[96vh] ${fullscreen ? "max-h-full max-w-full" : "rounded-xl sm:rounded-2xl"}`}><header className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-200 px-3 py-2.5 sm:px-4 sm:py-3"><div className="min-w-0"><h2 id="document-viewer-title" className="truncate text-sm font-bold text-slate-900">{document.name}</h2><p className="mt-1 truncate text-xs text-slate-500">{document.category} · {formatLabels[document.type]}</p></div><div className="flex shrink-0 items-center gap-1"><a href={url} download={filenameFor(document)} className="viewer-action" aria-label={`Download ${document.name}`}><Download className="size-4" /> <span className="hidden sm:inline">Download</span></a><button type="button" title={fullscreen ? "Exit fullscreen" : "Fullscreen"} aria-label={fullscreen ? "Exit fullscreen" : "Fullscreen"} onClick={() => setFullscreen((value) => !value)} className="viewer-action"><Maximize2 className="size-4" /></button><button type="button" onClick={onClose} className="viewer-action" aria-label="Close document viewer"><X className="size-5" /></button></div></header><main className="min-h-0 flex-1">{preview}</main><footer className="flex shrink-0 items-center justify-between gap-2 border-t border-slate-200 px-3 py-2 text-xs text-slate-500 sm:px-4"><span className="truncate">Original {formatLabels[document.type]} preserved</span>{officeTypes.has(document.type) && <a href={url} download={filenameFor(document)} className="shrink-0 font-semibold text-slate-700 underline">Download original</a>}</footer></div></div>
}
